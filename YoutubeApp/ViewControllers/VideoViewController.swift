...
class VideoViewController: UIViewController {
    ...
    @objc private func panVideoImageView(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .changed {
            ...
            let bottomMoveY = pararantViewHeight - videoImageMaxY
            let bottomMoveRatio = bottomMoveY / videoImageMaxY
            let bottomViewMoveConstant = move.y * bottomMoveRatio
            backViewBottomConstraint.constant = bottomViewMoveConstant
            
            // alpha値の設定
            let alphaRatio = move.y / (pararantViewHeight / 2)
